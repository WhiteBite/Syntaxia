...
3. 🔖 Редизайн Окна «Пресеты» (Скрин 4)
   ...
   Все основные кнопки («Сохранить») во всех модалках делаем градиентными, как на главном экране."# Syntaxia — AI Worker Factory

## Концепция

Syntaxia — система автоматизации разработки через AI-агентов. На вход подаётся задача (проект, фича, баг), система автоматически выполняет её через настроенные пайплайны.
