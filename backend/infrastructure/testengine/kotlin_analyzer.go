...
// KotlinTestAnalyzer implements domain.TestAnalyzer for Kotlin
type KotlinTestAnalyzer struct {
	log domain.Logger
}

// NewKotlinTestAnalyzer creates a new Kotlin test analyzer
func NewKotlinTestAnalyzer(log domain.Logger) *KotlinTestAnalyzer {
	return &KotlinTestAnalyzer{log: log}
}

// AnalyzeTestDependencies analyzes test dependencies
func (a *KotlinTestAnalyzer) AnalyzeTestDependencies(ctx context.Context, testPath string) ([]string, error) {
	a.log.Info(fmt.Sprintf("Analyzing dependencies for Kotlin test: %s", testPath))
...
func (a *KotlinTestAnalyzer) FindTestsForFile(ctx context.Context, filePath, projectPath string) ([]string, error) {
	a.log.Info(fmt.Sprintf("Finding Kotlin tests for file: %s", filePath))

	var testFiles []string

	// Get class name from file
	className := strings.TrimSuffix(filepath.Base(filePath), ".kt")
	...
	for _, testDir := range testDirs {
		...
		if strings.HasPrefix(relDir, "src/main/kotlin/") {
			relDir = strings.TrimPrefix(relDir, "src/main/kotlin/")
		} else if strings.HasPrefix(relDir, "src/main/java/") {
			relDir = strings.TrimPrefix(relDir, "src/main/java/")
		}

		testFile := filepath.Join(testDir, relDir, className+"Test.kt")
		if a.fileExists(filepath.Join(projectPath, testFile)) {
			testFiles = append(testFiles, testFile)
		}

		testFile2 := filepath.Join(testDir, relDir, className+"Tests.kt")
		if a.fileExists(filepath.Join(projectPath, testFile2)) {
			testFiles = append(testFiles, testFile2)
		}

		testFile3 := filepath.Join(testDir, relDir, className+"Spec.kt")
...
func (a *KotlinTestAnalyzer) IsSmokeTest(ctx context.Context, testPath string) (bool, error) {
	content, err := os.ReadFile(testPath)
	if err != nil {
...
func (a *KotlinTestAnalyzer) extractImports(content string) []string {
	var imports []string
	importSet := make(map[string]bool)

	// Match import statements
	importRe := regexp.MustCompile(`import\s+([a-zA-Z_][\w.]*(?:\.\*)?)\s*`)
...
func (a *KotlinTestAnalyzer) findTestsImportingClass(ctx context.Context, className, projectPath string) ([]string, error) {
	var testFiles []string
	...
	for _, testDir := range testDirs {
		...
		err := filepath.Walk(fullTestDir, func(path string, info os.FileInfo, err error) error {
			...
			if info.IsDir() || !strings.HasSuffix(info.Name(), ".kt") {
				return nil
			}
...
func (a *KotlinTestAnalyzer) importsClass(content, className string) bool {
	// Check for direct import
	pattern := fmt.Sprintf(`import\s+[\w.]*\.%s\s*`, className)
...
func (a *KotlinTestAnalyzer) fileExists(path string) bool {
	_, err := os.Stat(path)
	return err == nil
}

// removeDuplicates removes duplicate strings from slice
func (a *KotlinTestAnalyzer) removeDuplicates(items []string) []string {
