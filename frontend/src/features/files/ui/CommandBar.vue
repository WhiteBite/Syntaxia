<template>
  <div class="magic-bar-wrapper px-2 pb-2">
    <!-- Compact single-row MAGIC CONTROL BAR -->
    <div class="magic-bar flex items-center bg-[#1c1f2e] border border-white/10 rounded-xl p-1 shadow-2xl h-11 relative overflow-visible">
      ...
      <!-- CENTER: Stats, Recommendations & Selection -->
      <div class="flex-1 flex items-center justify-around px-2 min-w-0">
        <!-- Selected Counter & Token Estimate -->
        <div class="flex flex-col items-center justify-center min-w-[70px]" :title="t('commandBar.selected')">
          <span class="text-[10px] font-black text-indigo-400 leading-none">{{ selectedCount }}</span>
          <span class="text-[8px] uppercase text-gray-500 font-bold tracking-tighter">
            {{ selectedCount > 0 ? `~${estimatedTokens}k tok` : t('files.selected') }}
          </span>
        </div>

        <!-- Token Stats (Heatmap Dots) -->
        ...
        <button 
          ...
          :class="noiseReductionEnabled ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-500 hover:text-gray-300'"
      ...
      <div class="pl-1 border-l border-white/10 flex items-center">
        ...
        <button 
          v-if="selectedCount > 0"
          class="mr-2 p-1 text-gray-600 hover:text-red-400 transition-colors"
          @click="handleClear"
...
const recommendationsCount = computed(() => {
  ...
  return (analysis.relatedCount?.value || 0) + (analysis.dependentCount?.value || 0)
})
